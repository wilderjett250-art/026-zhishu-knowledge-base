...
WeFlow 属于数据源适配层。连接器仅访问本机 API，不保存 Token；用户选择的会话经过同步工作流进入客户领域，原始 ChatLab 响应保存为哈希快照，标准化消息进入客户消息表和独立全文索引。
...
- WeFlow 连接只允许 `127.0.0.1`、`localhost` 或 `::1`；Access Token 不持久化。
- 微信聊天默认 `restricted`，只同步用户在会话清单中明确勾选的客户。
