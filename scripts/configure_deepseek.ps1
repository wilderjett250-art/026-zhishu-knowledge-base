...
if ($SelfTest) {
    $shownHandler = {
        $keyTextBox.Text = "sk-pkas-synthetic-self-test-20260806"
...
if ($SelfTest) {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK -and $selfTestEnvPath) {
        $saved = [System.IO.File]::ReadAllText($selfTestEnvPath) -match '^PKAS_DEEPSEEK_API_KEY=sk-pkas-synthetic-self-test-20260806'
    }
