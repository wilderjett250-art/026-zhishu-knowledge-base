param(
    ...
    [int]$Port = 8765,
    [string]$TaskName = 'PKAS-Dashboard'
)
...
$runnerPath = Join-Path $resolvedRoot 'scripts\run_dashboard.ps1'
$pythonPath = Join-Path $resolvedRoot '.venv\Scripts\python.exe'
foreach ($requiredPath in @($runnerPath, $pythonPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "PKAS Dashboard runtime is incomplete: $requiredPath"
    }
}

$currentUser = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name
$powershellPath = "$env:SystemRoot\System32\WindowsPowerShell\v1.0\powershell.exe"
$arguments = "-NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$runnerPath`" -ProjectRoot `"$resolvedRoot`" -DataRoot `"$DataRoot`" -QdrantUrl `"$QdrantUrl`" -Port $Port"
...
$task = New-ScheduledTask -Action $action -Trigger $trigger -Principal $principal `
    -Settings $settings -Description 'PKAS localhost-only API and management dashboard.'
