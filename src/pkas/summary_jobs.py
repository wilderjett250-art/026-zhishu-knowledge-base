...
class SummaryJobRequest(BaseModel):
    ...
    scope: Literal["auto", "path", "all_local_drives", "catalog_batch"] = "auto"
...
class SummaryPromotion(BaseModel):
    ...
    mode: Literal["catalog", "full", "semantic"] = "full"
...
class SummaryJobs:
    def __init__(self, settings):
        ...
        self.active: dict[str, tuple[threading.Thread, threading.Event]] = {}
    ...
    def create(self, request: SummaryJobRequest, start=True):
        ...
        with sqlite3.connect(folder / "queue.sqlite") as db:
            ...
            meta = {
                ...
                "stage": "inspect" if request.scope == "catalog_batch" else "discover",
                "state": "paused",
        ...
        if request.scope == "catalog_batch":
            rows = self.catalog_ledger.reserve(job, request.catalog_batch_size)
            if not rows:
                raise ValueError("A库没有待分类或已变化的文件；无需新建批次")
    ...
    def catalog_promotion_selection(self, payload: SummaryPromotion) -> dict:
        ...
        return self.catalog_ledger.selection(payload.category_ids, payload.mode)
    ...
    def view(self, job, offset=0, limit=50):
        with self.db(job) as db:
            ...
            meta["running"] = job in self.active and self.active[job][0].is_alive()
    ...
    def resume(self, job):
        with self.guard:
            ...
            thread = threading.Thread(target=self._run, args=(job, stop, lock), daemon=True)
            self.active[job] = (thread, stop)
            thread.start()
        ...
        with self.guard:
            ...
            with WindowsFileLock(self.directory(job) / "worker.lock"), self.db(job) as db:
                ...
                db.execute(
                    ...
                    "WHERE state='error' OR ai_state='stale'"
                )
    ...
    def close(self):
        for _, stop in self.active.values():
            stop.set()
        for thread, _ in self.active.values():
    ...
    def _inspect(self, job, stop):
        ...
        while not stop.is_set():
            ...
            for row in rows:
                ...
                summary = {
                    "text": "本地抽样登记；用途建议：" + record["classification"]["label"],
                    "origin": "local",
    ...
    def _validate_agent_result(packet, result, categories):
        ...
        for item in items:
            ...
            if any(
                not isinstance(item.get(key), str) or len(item[key]) > 2000
                for key in ("summary", "evidence", "uncertainty")
            ):
            ...
            for _ in range(request["max_ai_batches"]):
                ...
                try:
                    ...
                    retry_packet["validation_feedback"] = (
                        ...
                        "category_id必须来自给定分类；summary、evidence、uncertainty均为字符串。"
                        "evidence必须逐字、连续存在于对应sample中且不超过40字符；"
                        "否则选择unresolved_other并返回空evidence。"
                        "这是唯一一次纠错重试。"
                    )
                ...
                # Bound conversation growth: one fresh thread per 10 batches, not per file.
                if (_ + 1) % 10 == 0:
