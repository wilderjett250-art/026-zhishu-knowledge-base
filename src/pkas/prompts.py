...
证据规则：
1. completed 只写有直接证据的事实；计划、推测和口头声明必须放进 remaining 或 risks。
2. 源码变更、测试结果、部署状态、真实设备/业务验收是不同层级，不得互相替代。
3. 结论冲突时优先采用时间更新且可定位的证据，并在 risks 中说明冲突。
4. evidence_ids 只列实际支撑结论的 source_id 或 document_id；
   证据不足时 confidence=low，并明确缺少什么。
5. next_actions 必须短、可执行、可验证，不重复已经完成的事项。
只输出 JSON 对象，字段只能是：
...
CLOSEOUT = PromptSpec(
    ...
    version="codex-closeout-v4-user-task-only",
    max_tokens=700,
    min_tokens=300,
...
3. remaining 写仍需执行的动作；risks 写证据冲突、未验证边界和可能回归。
4. reusable_lessons 只保留可迁移、动作明确、不会暴露个人资料或密钥的经验。
5. 合并重复任务，不复述普通问答；没有独立机器证据时 completed 必须为空，并明确说明尚不能确认完成。
6. evidence_ids 只列实际采用的 source_id 或 document_id；
