...
const pageMeta: Record<Page, { eyebrow: string; title: string; intro: string }> = {
  ...
  import: { eyebrow: "SOURCE GATE", title: "先看边界，再接入资料", intro: "只有你明确指定的路径会进入知识库，密钥类文件始终跳过。" },
...
function App() {
  ...
  const refresh = useCallback(async () => {
    const [dashboardResult, healthResult, sourcesResult, definitionsResult, runsResult,
      agentResult, personaResult, distillationResult, auditResult] = await Promise.allSettled([
    ...
    if ([dashboardResult, healthResult, sourcesResult, definitionsResult, runsResult,
      agentResult, personaResult, distillationResult, auditResult].some((result) => result.status === "rejected")) {
...
function Home({ dashboard, setPage, setEvidence }: { dashboard: Json | null; setPage: (page: Page) => void; setEvidence: (data: Json) => void }) {
  ...
  return <>
    ...
    <section className="metric-grid">
      <Metric label="资料来源" value={counts.sources ?? 0} unit="SOURCE" tone="cyan" />
      <Metric label="知识片段" value={counts.chunks ?? 0} unit="CHUNK" tone="violet" />
      <Metric label="个人观察" value={counts.persona_observations ?? 0} unit="SIGNAL" tone="amber" />
      <Metric label="蒸馏样本" value={counts.distillation_examples ?? 0} unit="SAMPLE" tone="green" />
    </section>
...
function EvidenceSpine({ evidence, close }: { evidence: Json | null; close: () => void }) {
  ...
  return <aside className="evidence-spine"><header><div><small>EVIDENCE SPINE</small><strong>来源与原文</strong></div><button onClick={close}>×</button></header><div className="spine-body"><span className="spine-index">SOURCE / VERIFIED</span><h2>{evidence.title ?? evidence.original_name ?? "证据详情"}</h2>{evidence.text && <pre>{evidence.text}</pre>}{evidence.snippet && !evidence.text && <p className="evidence-quote">{evidence.snippet}</p>}<dl className="detail-list"><Detail label="资料 ID" value={evidence.source_id ?? evidence.id ?? "—"} /><Detail label="文档 ID" value={evidence.document_id ?? (evidence.id?.startsWith?.("doc_") ? evidence.id : "—")} /><Detail label="原始路径" value={evidence.original_uri ?? "—"} /><Detail label="仓内原件" value={evidence.vault_path ?? "—"} /><Detail label="定位" value={evidence.locator ?? `字符 ${evidence.offset ?? 0} 起`} /><Detail label="领域 / 隐私" value={`${evidence.domain ?? "—"} / ${evidence.privacy ?? "—"}`} /></dl></div></aside>;
}
...
function workflowName(value: string) { return ({ import_path: "资料导入与索引", rebuild_search_index: "全文索引重建", persona_review: "个人观察审核" } as Record<string, string>)[value] ?? value; }
function approvalLabel(value: string) { return ({ candidate: "待审核", approved: "已批准", rejected: "已驳回" } as Record<string, string>)[value] ?? value; }
function eventLabel(value: string) { return ({ source_ingested: "资料已接入", persona_candidate_created: "画像候选已创建", persona_reviewed: "画像已审核", distillation_candidate_created: "蒸馏候选已创建", distillation_reviewed: "蒸馏样本已审核" } as Record<string, string>)[value] ?? value; }
