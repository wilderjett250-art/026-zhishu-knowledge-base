...
const nav: Array<{ id: Page; label: string; mark: string }> = [
  ...
  { id: "rag", label: "RAG 实验室", mark: "RG" },
  { id: "customers", label: "微信会话", mark: "CX" },
  { id: "timeline", label: "每日足迹", mark: "DAY" },
...
const pageMeta: Record<Page, { eyebrow: string; title: string; intro: string }> = {
  ...
  rag: { eyebrow: "RETRIEVAL OBSERVATORY", title: "看见向量，量化每次召回", intro: "真实向量覆盖、语义聚类、检索证据和黄金集指标都在同一个实验台。" },
  customers: { eyebrow: "CUSTOMER INTELLIGENCE", title: "先归类，再进入客户业务闭环", intro: "导入的聊天先作为待确认微信会话；人工确认为客户后，Codex 才能准备回复和业务信号。" },
  timeline: { eyebrow: "PERSONAL TIMELINE", title: "每天做了什么，手里正在忙什么", intro: "只整理本人发送的聊天；事实、推断和待办分开，并保留原消息证据。" },
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-foot">
          ...
          <strong>{health ? "ONLINE" : ["runtime", "readiness", "foundation"].includes(page) ? "见当前页面" : "未检查"}</strong>
...
function Home({ dashboard, setPage, setEvidence, run }: { dashboard: Json | null; setPage: (page: Page, foundationTab?: "intake") => void; setEvidence: (data: Json) => void; run: Runner }) {
  ...
  return <>
    ...
    <section className="metric-grid">
      ...
      <Metric label="知识片段" value={counts.knowledge_chunks ?? 0} unit="KNOWLEDGE" tone="green" />
    </section>
...
function CustomersPanel({ customers, signals, run, setEvidence, busy }: { customers: Json[]; signals: Json[]; run: Runner; setEvidence: (data: Json) => void; busy: boolean }) {
  ...
  if (!customers.length) return <div className="customer-empty"><span>CX / 00</span><h2>还没有微信会话</h2><p>进入“WeFlow / 资料”，读取 WeFlow 已导出的 XLSX。导入后会先进入待归类区，不会自动认定为客户。</p></div>;
...
function ImportPanel({ run, busy }: { run: Runner; busy: boolean }) {
  ...
  return <>
    <section className="manual-weflow-sync">
      <div>
        <small>MANUAL / INCREMENTAL</small>
        <h2>一键同步微信新消息</h2>
...
function PersonalTimeline({ run, busy, setEvidence }: { run: Runner; busy: boolean; setEvidence: (data: Json) => void }) {
  ...
  return <section className="timeline-layout">
    <aside className="timeline-days">
      <header><small>DAILY INDEX</small><strong>{days.length} 天</strong></header>
    ...
    <div className="timeline-detail">
      {!day ? <div className="timeline-empty"><strong>{date}</strong><p>这一天还没有整理。整理后会把事实、忙碌方向和待办分开。</p></div> : <>
        <header className="timeline-summary"><div><small>FACTUAL SUMMARY</small><h2>{day.local_date}</h2><p>{day.factual_summary || "没有形成可证实摘要。"}</p></div><div><strong>{day.evidence_count}</strong><span>条采用证据</span><small>{day.candidate_message_count} / {day.source_message_count} 条候选</small></div></header>
        <section className="timeline-section"><header><h3>有聊天证据的事项</h3><small>FACTS</small></header><div className="timeline-cards">
        ...
        <section className="timeline-split">
          <div><header><h3>可能正在忙</h3><small>INFERENCE · 非事实</small></header>{(day.inferred_focus ?? []).map((item: Json, index: number) => <article key={index}><p>{item.statement}</p><span>{confidenceLabel(item.confidence)}置信度 · 仅作推断</span><footer>{(item.evidence_ids ?? []).map((id: string) => <button key={id} onClick={() => showEvidence(id)}>查看推断依据</button>)}</footer></article>)}</div>
          <div><header><h3>明确待办</h3><small>OPEN ITEMS</small></header>{(day.open_items ?? []).map((item: Json, index: number) => <article key={index}><p>{item.statement}</p><span>{confidenceLabel(item.confidence)}置信度</span><footer>{(item.evidence_ids ?? []).map((id: string) => <button key={id} onClick={() => showEvidence(id)}>查看消息证据</button>)}</footer></article>)}</div>
        </section>
      </>}
    </div>
  </section>;
