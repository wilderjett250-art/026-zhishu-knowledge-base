...
export default function ClassifiedIntake() {
  ...
  return <section className="foundation-panel intake-panel">
    <header><div><small>资料接入</small><h2>选择资料范围</h2></div><span className="intake-badge">原件留在原处</span></header>
    <section className="processing-profile-picker" aria-label="资料处理方案">
      <header><div><small>整理方式</small><strong>先选整理方式，再扫描资料</strong><p>前三种方案使用预设规则；自定义方案会先完成 AI 速判和摘要，再由你确认哪些内容进入知识库。</p></div><span>{profiles?.current?.label ?? "自定义方案"}</span></header>
