...
type Gate = { status: string; metrics: Record<string, unknown>; evidence: string; next_action: string; checked_at: string; cached: boolean };
const sections = [
...
function CheckCard({ id, title, refreshKey }: { id: string; title: string; refreshKey: number }) {
  ...
  return <article className={error ? "blocked" : busy ? "unknown" : result?.status ?? "unknown"} aria-busy={busy}>
    <header><h3>{title}</h3><span>{busy ? "检查中" : error ? "暂未完成" : labels[result?.status ?? "unknown"]}</span></header>
    {busy && <p role="status">正在读取本项状态，结果将独立显示。</p>}
    {error && <p role="alert">{error}</p>}
    {result && <>
      {(busy || error) && <p>以下是上一次检查结果，不代表当前状态。</p>}
