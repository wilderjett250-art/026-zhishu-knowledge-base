  ...
  return <>
    ...
    <section className="rag-grid">
      <article className="vector-panel">
        <header><div><small>VECTOR CELL ATLAS</small><h2>知识细胞图</h2></div><span>{map.sampled ?? 0} / 300 SAMPLE</span></header>
    ...
    <section className="search-lab">
      <header><div><small>SEARCH GROUND TRUTH</small><h2>检索与标注实验台</h2></div>{latest && <div className="latest-metrics"><span>Hit {percent(latest.hit_rate)}</span><span>标注P@5 {percent(latest.precision_at_k)}</span><span>nDCG {Number(latest.ndcg_at_k ?? 0).toFixed(3)}</span><span>覆盖 {percent(latest.judgment_coverage ?? 0)}</span></div>}</header>
      ...
      {retrievalHealth && <div className={`retrieval-health ${retrievalHealth.sufficiency}`}><div><small>RETRIEVAL HEALTH</small><strong>{healthLabel(retrievalHealth.sufficiency)}</strong><span>{actualRetrievalMode} · 候选 {retrievalHealth.candidate_count} · 返回 {retrievalHealth.result_count} · 来源 {retrievalHealth.source_count}</span></div><dl><Row label="检索策略" value={queryPlanLabel(retrievalHealth.query_plan)} /><Row label="策略候选" value={retrievalHealth.query_plan?.candidate_limit ?? "—"} /><Row label="Catalog" value={retrievalHealth.channels?.catalog ?? 0} /><Row label="FTS" value={retrievalHealth.channels?.fts ?? 0} /><Row label="Vector" value={retrievalHealth.channels?.vector ?? 0} /><Row label="Rerank" value={retrievalHealth.rerank_status} /><Row label="覆盖率" value={percent(retrievalHealth.vector_coverage)} /></dl>{retrievalHealth.query_plan?.reasons?.length > 0 && <p>{retrievalHealth.query_plan.reasons.join("；")}</p>}{retrievalHealth.reasons?.length > 0 && <p>{retrievalHealth.reasons.join("；")}</p>}</div>}
      <div className="lab-results">
        {results.map((item, index) => <article key={`${item.source_id}-${index}`} className={`${(grades[item.source_id] ?? -1) > 0 ? "selected" : ""} ${item.seeded && !item.retrieved ? "seeded-source" : ""}`}>
          ...
          <button onClick={() => setEvidence(item)}><b>{item.seeded && !item.retrieved ? "SEED" : `#${item.rank ?? index + 1}`}</b><span><strong>{item.title}</strong><p>{item.snippet || "该来源当前没有可显示的片段，请通过证据抽屉核对来源状态。"}</p><small>{item.seeded && !item.retrieved ? "当前 Top10 未召回 · 必须人工核对" : `${item.match_strategy ?? "source"} · ${item.domain ?? "—"}`} · {item.locator ?? item.source_id}</small></span></button>
        </article>)}
